...
import { getAllPostData, getFreelancerAcceptedJobs } from "@/blockchain/utils";
  ...
  return (
    <section className="mt-4 font-outfit">
      <div className="grid grid-cols-1 gap-8 mt-8 md:grid-cols-2 lg:grid-cols-3">
        ...
        {postings.filter(
          // @ts-ignore
          (post) => post.freelancerId === address && !post.completed
        ).length > 0 ? (
          postings?.map((post, idx) => {
            // @ts-ignore
            if (post?.freelancerId === address && !post.completed)
              return (
                <div
